            ...
            try:
                ...
                if github_token:
                    ...
                    if counter_response.status_code == 200:
                        ...
                        counter_content = base64.b64decode(counter_data['content']).decode('utf-8')
                    ...
                    if file_response.status_code in [200, 201]:
                        ...
                        counter_content_encoded = base64.b64encode(new_counter.encode('utf-8')).decode('utf-8')
                        
                        counter_payload = {
                            "message": f"Update counter to {new_counter}",
                            "content": counter_content_encoded,
                            "branch": "main"
                        }
                        if counter_sha:
                        ...
                        st.warning(f"⚠️ GitHub 저장 실패: {file_response.status_code}")
                else:
                    st.info("💡 GitHub 토큰이 설정되지 않아 자동 저장이 비활성화되었습니다.")
